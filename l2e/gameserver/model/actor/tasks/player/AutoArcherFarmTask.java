...
public class AutoArcherFarmTask implements Runnable {
   ...
   private boolean selectRandomTarget() {
      if (this._player.isCastingNow()) {
         ...
         if (this._committedOwner != null) {
            ...
            if (this._committedTarget != null) {
               return true;
            } else if (Math.sqrt(this._player.getDistanceSq(this._committedOwner)) > 300.0) {
               Location loc = Location.findPointToStay(this._committedOwner.getLocation(), 300, this._player.getGeoIndex(), false);
               if (loc != null) {
                  this._player.getAI().setIntention(CtrlIntention.MOVING, loc);
               }
