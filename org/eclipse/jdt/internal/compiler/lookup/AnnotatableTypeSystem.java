...
public class AnnotatableTypeSystem extends TypeSystem {
   ...
   public ReferenceBinding getMemberType(ReferenceBinding memberType, ReferenceBinding enclosingType) {
      return !this.haveTypeAnnotations(memberType, enclosingType)
         ? super.getMemberType(memberType, enclosingType)
         : (ReferenceBinding)this.getAnnotatedType(memberType, enclosingType, memberType.getTypeAnnotations());
   }
      ...
      if (genericType.hasTypeAnnotations()) {
         ...
         for(int length = derivedTypes.length; i < length; ++i) {
            ...
            if (derivedType.isWildcard()
               ...
               && Util.effectivelyEqual(derivedType.additionalBounds(), otherBounds)) {
               if (Util.effectivelyEqual(derivedType.getTypeAnnotations(), annotations)) {
      ...
      if (type != null && type.isValidBinding() && annotations != null && annotations.length != 0) {
         ...
         switch(type.kind()) {
            ...
            case 32772:
               ...
               for(TypeBinding var14 = j == 0 ? null : types[j - 1]; i < levels; ++j) {
                  ...
                  annotatedType = this.getAnnotatedType(currentType, (TypeBinding)var14, currentAnnotations);
                  var14 = annotatedType;
                  ++i;
               }
               break;
               ...
               switch(type.kind()) {
                  case 68:
                     if (!derivedType.isArrayType()
                        ...
                        || derivedType.leafComponentType() != type.leafComponentType()) {
                        continue;
                     }
                     break;
                  case 516:
                  case 8196:
                     if (!derivedType.isWildcard()
                        ...
                        || derivedType.boundKind() != type.boundKind()
                        || derivedType.bound() != type.bound()
                        || !Util.effectivelyEqual(derivedType.additionalBounds(), type.additionalBounds())) {
                        continue;
                     }
                     break;
                  case 1028:
   ...
   private boolean haveTypeAnnotations(TypeBinding baseType, TypeBinding someType, TypeBinding[] someTypes, AnnotationBinding[] annotations) {
      if (baseType != null && baseType.hasTypeAnnotations()) {
         ...
         for(int length = someTypes == null ? 0 : someTypes.length; i < length; ++i) {
