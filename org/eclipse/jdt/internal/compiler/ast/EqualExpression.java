...
public class EqualExpression extends BinaryExpression {
   ...
   private void checkNullComparison(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, FlowInfo initsWhenTrue, FlowInfo initsWhenFalse) {
      ...
      if (!leftNonNullChecked) {
         ...
         if (local != null) {
            ...
            && (!contextualCheckEquality && rightStatus == 2 || contextualCheckEquality && rightStatus == 4)
      ...
      if (!rightNonNullChecked) {
         ...
         if (local != null) {
            ...
            && (!contextualCheckEquality && leftStatus == 2 || contextualCheckEquality && leftStatus == 4)
            && scope.compilerOptions().enableSyntacticNullAnalysisForFields) {
   ...
   private void checkVariableComparison(
      BlockScope scope,
   ...
   public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
      ...
      if ((this.bits & 4032) >> 6 == 18) {
         if (this.left.constant != Constant.NotAConstant && this.left.constant.typeID() == 5) {
            ...
            result = this.right
               .analyseCode(currentScope, flowContext, this.left.analyseCode(currentScope, flowContext, flowInfo).unconditionalInits())
               .unconditionalInits();
   ...
   public void generateOptimizedBooleanEqual(
      BlockScope currentScope, CodeStream codeStream, BranchLabel trueLabel, BranchLabel falseLabel, boolean valueRequired
   ) {
   ...
   public void generateNonBooleanEqual(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
      ...
      if ((this.left.implicitConversion & 0xFF) >> 4 == 10) {
         ...
         if (this.left.constant != Constant.NotAConstant && cst.intValue() == 0) {
            this.right.generateCode(currentScope, codeStream, valueRequired);
            if (valueRequired) {
               BranchLabel falseLabel = new BranchLabel(codeStream);
               if (isEqualOperator) {
         ...
         if (this.right.constant != Constant.NotAConstant && cst.intValue() == 0) {
            this.left.generateCode(currentScope, codeStream, valueRequired);
            if (valueRequired) {
               BranchLabel falseLabel = new BranchLabel(codeStream);
               if (isEqualOperator) {
   ...
   public void generateOptimizedNonBooleanEqual(
      ...
      if (this.right.constant != Constant.NotAConstant && (this.left.implicitConversion & 0xFF) >> 4 == 10 && inline.intValue() == 0) {
         this.left.generateCode(currentScope, codeStream, valueRequired);
         if (valueRequired) {
            if (falseLabel == null) {
               if (trueLabel != null) {
         ...
         if (this.left.constant != Constant.NotAConstant && (this.left.implicitConversion & 0xFF) >> 4 == 10 && inline.intValue() == 0) {
            this.right.generateCode(currentScope, codeStream, valueRequired);
            if (valueRequired) {
               if (falseLabel == null) {
                  if (trueLabel != null) {
   ...
   public TypeBinding resolveType(BlockScope scope) {
      ...
      if (originalLeftType != null && originalRightType != null) {
            ...
            if ((operatorSignature & 15) == 0) {
               ...
               if (leftIsCast || rightIsCast) {
                  CastExpression.checkNeedForArgumentCasts(
                     scope, 18, operatorSignature, this.left, leftType.id, leftIsCast, this.right, rightType.id, rightIsCast
                  );
               }
               ...
               if (leftDirect != null && leftDirect == Expression.getDirectBinding(this.right)) {
                  ...
                  if (operator == 18 && this.constant == BooleanConstant.fromValue(true)
                     || operator == 29 && this.constant == BooleanConstant.fromValue(false)) {
                     scope.problemReporter().comparingIdenticalExpressions(this);
                  }
               }
         ...
         } else if ((!leftType.isBaseType() || leftType == TypeBinding.NULL)
