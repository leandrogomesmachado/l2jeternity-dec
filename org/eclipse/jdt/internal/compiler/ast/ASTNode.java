...
public abstract class ASTNode implements TypeConstants, TypeIds {
   ...
   private static int checkInvocationArgument(
      BlockScope scope, Expression argument, TypeBinding parameterType, TypeBinding argumentType, TypeBinding originalParameterType
   ) {
   ...
   public final boolean isFieldUseDeprecated(FieldBinding field, Scope scope, int filteredBits) {
      if ((this.bits & 32768) == 0 && (filteredBits & 8192) == 0 && field.isOrEnclosedByPrivateType() && !scope.isDefinedInField(field)) {
         if ((filteredBits & 65536) != 0) {
            ++field.original().compoundUseFlag;
         } else {
            FieldBinding var5 = field.original();
            var5.modifiers |= 134217728;
         }
      }
      ...
      if ((field.modifiers & 262144) != 0) {
         ...
         if (restriction != null) {
            scope.problemReporter()
               .forbiddenReference(field, this, restriction.classpathEntryType, restriction.classpathEntryName, restriction.getProblemId());
         }
      }
   ...
   public final boolean isMethodUseDeprecated(MethodBinding method, Scope scope, boolean isExplicitUse) {
      if ((this.bits & 32768) == 0 && method.isOrEnclosedByPrivateType() && !scope.isDefinedInMethod(method)) {
      ...
      if (isExplicitUse && (method.modifiers & 262144) != 0) {
         ...
         if (restriction != null) {
            scope.problemReporter()
               .forbiddenReference(method, this, restriction.classpathEntryType, restriction.classpathEntryName, restriction.getProblemId());
         }
      }
   ...
   public final boolean isTypeUseDeprecated(TypeBinding type, Scope scope) {
      ...
      if (type.isBaseType()) {
         ...
         if ((this.bits & 32768) == 0 && refType instanceof TypeVariableBinding) {
            refType.modifiers |= 134217728;
         }

         if ((this.bits & 32768) == 0 && refType.isOrEnclosedByPrivateType() && !scope.isDefinedInType(refType)) {
         ...
         if (refType.hasRestrictedAccess()) {
            ...
            if (restriction != null) {
               scope.problemReporter()
                  .forbiddenReference(type, this, restriction.classpathEntryType, restriction.classpathEntryName, restriction.getProblemId());
            }
         }
   ...
   public static void resolvePolyExpressionArguments(Invocation invocation, MethodBinding method, TypeBinding[] argumentTypes, BlockScope scope) {
      ...
      if (candidateMethod != null) {
         ...
         if (variableArity
            ...
            && arguments[arguments.length - 1].isCompatibleWith(parameters[parameters.length - 1], scope)) {
            variableArity = false;
         }
      ...
      if (annotations != null && annotations.length != 0 && recipient != null) {
         ...
         if (se8Annotations != null) {
            switch(recipient.kind()) {
               case 1:
                  ...
                  if (Annotation.isTypeUseCompatible(fieldDeclaration.type, scope)) {
                     ...
                     fieldDeclaration.type.bits |= 1048576;
               ...
               case 2:
                  ...
                  if (Annotation.isTypeUseCompatible(typeRef, scope)) {
                     local.declaration.bits |= 1048576;
               ...
               case 8:
                  ...
                  if (!method.isConstructor()) {
                     SourceTypeBinding sourceType = (SourceTypeBinding)method.declaringClass;
                     ...
                     if (Annotation.isTypeUseCompatible(methodDecl.returnType, scope)) {
                        ...
                        methodDecl.returnType.bits |= 1048576;
                        method.returnType = mergeAnnotationsIntoType(
                           scope, se8Annotations, se8nullBits, se8NullAnnotation, methodDecl.returnType, method.returnType
                        );
                        if (scope.environment().usesNullTypeAnnotations()) {
            ...
            for(int ix = 0; ix < length; ++ix) {
               AnnotationBinding recipientAnnotation = recipientAnnotations[ix];
               if (recipientAnnotation != null) {
                  long annotationTargetMask = recipientAnnotation.getAnnotationType().getAnnotationTagBits() & 27039155590529024L;
            ...
            if (newLength != length) {
               AnnotationBinding[] var23;
               System.arraycopy(recipientAnnotations, 0, var23 = new AnnotationBinding[newLength], 0, newLength);
               recipient.setAnnotations(var23, scope);
            }
         }
      }
   }

   private static TypeBinding mergeAnnotationsIntoType(
      BlockScope scope, AnnotationBinding[] se8Annotations, long se8nullBits, Annotation se8NullAnnotation, TypeReference typeRef, TypeBinding existingType
   ) {
      if (existingType != null && existingType.isValidBinding()) {
         ...
         if (se8nullBits != 0L && oldLeafType.isBaseType()) {
            ...
            if (unionRef == null) {
               typeRef.resolvedType = (TypeBinding)(existingType.isArrayType()
                  ? scope.environment().createArrayType(newLeafType, existingType.dimensions(), existingType.getTypeAnnotations())
                  : newLeafType);
            } else {
               ...
               if (CharOperation.equals(TypeConstants.JAVA_LANG_DEPRECATED[2], annotationTypeRef.getLastToken())) {
                  ...
                  if (annotationType != null && annotationType.isValidBinding() && annotationType.id == 44) {
                     switch(kind) {
                        case 1:
                           FieldBinding field = (FieldBinding)recipient;
                           field.tagBits |= 70385924046848L;
                           return;
                        case 2:
                           LocalVariableBinding local = (LocalVariableBinding)recipient;
                           local.tagBits |= 70385924046848L;
                           return;
                        case 4:
                        case 2052:
                        case 4100:
                           ReferenceBinding type = (ReferenceBinding)recipient;
                           type.tagBits |= 70385924046848L;
                           return;
                        case 8:
                           MethodBinding method = (MethodBinding)recipient;
                           method.tagBits |= 70385924046848L;
                           return;
                        case 16:
                           PackageBinding packageBinding = (PackageBinding)recipient;
                           packageBinding.tagBits |= 70385924046848L;
                           return;
                        default:
                           return;
                     }
