...
public class SingleNameReference extends NameReference implements OperatorIds {
   ...
   public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, Assignment assignment, boolean isCompound) {
      boolean isReachable = (flowInfo.tagBits & 3) == 0;
      if (isCompound) {
         switch(this.bits & 7) {
            case 1:
      ...
      switch(this.bits & 7) {
         ...
         case 2:
            ...
            if (flowInfo.isPotentiallyAssigned(localBinding) || (this.bits & 524288) != 0) {
   ...
   public boolean checkNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, int ttlForFieldCheck) {
      if (!super.checkNPE(scope, flowContext, flowInfo, ttlForFieldCheck)) {
         ...
         if (compilerOptions.isAnnotationBasedNullAnalysisEnabled && this.binding instanceof FieldBinding) {
            return this.checkNullableFieldDereference(
               scope, (FieldBinding)this.binding, ((long)this.sourceStart << 32) + (long)this.sourceEnd, flowContext, ttlForFieldCheck
            );
         }
      }
   ...
   public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
      if (assignment.expression.isCompactableOperation()) {
         ...
         if (operation.right instanceof SingleNameReference
            ...
            && (operation.left.implicitConversion & 0xFF) >> 4 != 11
            && (operation.right.implicitConversion & 0xFF) >> 4 != 11) {
            variableReference.generateCompoundAssignment(
               currentScope,
               codeStream,
               this.syntheticAccessors == null ? null : this.syntheticAccessors[1],
               ...
               if (codegenField.isStatic()) {
                  ...
                  if (this.syntheticAccessors != null && this.syntheticAccessors[0] != null) {
                     ...
                     TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(
                        currentScope, codegenField, this.actualReceiverType, true
                     );
         ...
         if (this.constant == Constant.NotAConstant) {
            if ((this.bits & 1) != 0) {
               ...
               if ((this.bits & 8160) != 0
                  ...
                  this.syntheticAccessors[isReadAccess ? 0 : 1] = ((SourceTypeBinding)currentScope.enclosingSourceType()
                        .enclosingTypeAt((this.bits & 8160) >> 5))
                     .addSyntheticMethod(codegenField, isReadAccess, false);
