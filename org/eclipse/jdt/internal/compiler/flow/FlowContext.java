...
public class FlowContext implements TypeConstants {
                  ...
                  switch(state) {
                     case -1:
                        exceptionContext.recordHandlingException(
                           caughtException, exceptionFlow.unconditionalInits(), raisedException, raisedException, location, definitelyCaught
                        );
                     ...
                     case 1:
                        exceptionContext.recordHandlingException(
                           caughtException, exceptionFlow.unconditionalInits(), raisedException, caughtException, location, false
                        );
                  }
               }
   ...
   public void checkExceptionHandlers(TypeBinding[] raisedExceptions, ASTNode location, FlowInfo flowInfo, BlockScope scope) {
      ...
      if (raisedExceptions != null && (raisedCount = raisedExceptions.length) != 0) {
         ...
         TypeBinding[] var23;
         System.arraycopy(raisedExceptions, 0, var23 = new TypeBinding[raisedCount], 0, raisedCount);
         FlowContext traversedContext = this;
         ...
         for(ArrayList abruptlyExitedLoops = null; traversedContext != null; traversedContext = traversedContext.getLocalParent()) {
            ...
            if (traversedContext instanceof ExceptionHandlingFlowContext) {
               ...
               if (exceptionContext.handledExceptions != Binding.NO_EXCEPTIONS) {
                  ...
                  for(int caughtIndex = 0; caughtIndex < caughtCount; ++caughtIndex) {
                     ...
                     for(int raisedIndex = 0; raisedIndex < raisedCount; ++raisedIndex) {
                        ...
                        if ((raisedException = var23[raisedIndex]) != null) {
                           FlowInfo exceptionFlow = flowInfo;
                           int state = caughtException == null ? -1 : Scope.compareTypes(raisedException, caughtException);
                           if (abruptlyExitedLoops != null && caughtException != null && state != 0) {
                              int i = 0;
               ...
               if (exceptionContext.isMethodContext) {
                  for(int i = 0; i < raisedCount; ++i) {
                     TypeBinding raisedException;
                     if ((raisedException = var23[i]) != null && raisedException.isUncheckedException(false)) {
                        --remainingCount;
                        var23[i] = null;
                     }
                  }
                  ...
                  if (shouldMergeUnhandledException) {
                     for(int i = 0; i < raisedCount; ++i) {
                        TypeBinding raisedException;
                        if ((raisedException = var23[i]) != null) {
                           exceptionContext.mergeUnhandledException(raisedException);
                        }
                     }
         ...
         for(int i = 0; i < raisedCount; ++i) {
            ...
            if ((exception = var23[i]) != null) {
               for(int j = 0; j < i; ++j) {
                  if (TypeBinding.equalsEquals(var23[j], exception)) {
                     continue label121;
                  }
               }
   ...
   public FlowContext getTargetContextForBreakLabel(char[] labelName) {
      ...
      for(FlowContext lastNonReturningSubRoutine = null; current != null; current = current.getLocalParent()) {
         ...
         if ((currentLabelName = current.labelName()) != null && CharOperation.equals(currentLabelName, labelName)) {
            ((LabeledStatement)current.associatedNode).bits |= 64;
   ...
   public FlowContext getLocalParent() {
      return !(this.associatedNode instanceof AbstractMethodDeclaration)
      ...
      if (providedType != null) {
         ...
         if (annotationStatus != null) {
            ...
            currentScope.problemReporter()
               .nullityMismatch(expression, providedType, expectedType, nullStatus, currentScope.environment().getNonNullAnnotationName());
         }
      }
   }
