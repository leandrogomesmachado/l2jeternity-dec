...
public final class CharOperation {
   ...
   public static final char[] append(char[] array, char suffix) {
      if (array == null) {
         ...
         char[] var3;
         System.arraycopy(array, 0, var3 = new char[length + 1], 0, length);
         var3[length] = suffix;
         return var3;
      }
   }
   ...
   public static final int indexOf(char[] toBeFound, char[] array, boolean isCaseSensitive, int start, int end) {
      ...
      if (toBeFoundLength > end || start < 0) {
         return -1;
      } else if (toBeFoundLength == 0) {
         return 0;
      } else if (toBeFoundLength == end) {
         if (isCaseSensitive) {
         ...
         if (isCaseSensitive) {
            ...
            label95:
            for(int max = end - toBeFoundLength + 1; i < max; ++i) {
   ...
   public static final boolean match(char[] pattern, int patternStart, int patternEnd, char[] name, int nameStart, int nameEnd, boolean isCaseSensitive) {
      if (name == null) {
         ...
         for(char patternChar = '\u0000'; iPattern != patternEnd; ++iPattern) {
            if ((patternChar = pattern[iPattern]) == '*') {
               ...
               while(iName < nameEnd) {
                  if (iPattern == patternEnd) {
                     ...
                     iName = ++prefixStart;
                  } else if ((patternChar = pattern[iPattern]) == true) {
                     segmentStart = ++iPattern;
                     if (iPattern == patternEnd) {
                        return true;
                     }
      ...
      if (array == null) {
         ...
         if (length == 0) {
            ...
            if (result == null) {
               ...
               char[] var7;
               System.arraycopy(result, 0, var7 = new char[count], 0, count);
               return var7;
            }
         }
      }
