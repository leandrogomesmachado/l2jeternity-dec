   ...
   public boolean useItem(Playable playable, ItemInstance item, boolean forceUse) {
      if (!playable.isPlayer()) {
         ...
         if (!activeOwner.hasSummon()) {
            ...
            int shotConsumption = activeOwner.getSummon().getSpiritShotsPerHit();
            ...
            if (skills == null) {
               ...
               if (shotCount < (long)shotConsumption) {
                  if (!activeOwner.haveAutoShot(itemId)) {
                     activeOwner.sendPacket(SystemMessageId.NOT_ENOUGH_SPIRITHOTS_FOR_PET);
                  }

                  return false;
               } else if (activeOwner.getSummon().isChargedShot(isBlessed ? ShotType.BLESSED_SPIRITSHOTS : ShotType.SPIRITSHOTS)) {
                  return false;
               } else if (!Config.INFINITE_BEAST_SPIRIT_SHOT
