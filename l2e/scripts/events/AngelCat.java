...
public class AngelCat extends AbstractWorldEvent {
   ...
   public AngelCat(String name, String descr) {
      super(name, descr);
      this.addStartNpc(4308);
      this.addFirstTalkId(4308);
      this.addTalkId(4308);
   ...
   public boolean eventStart(long totalTime) {
      if (!this._isActive && totalTime != 0L) {
         ...
         ServerMessage msg = new ServerMessage("EventAngelCat.START", true);
   ...
   public boolean eventStop() {
      if (!this._isActive) {
         ...
         this._npcList.clear();
         ServerMessage msg = new ServerMessage("EventAngelCat.STOP", true);
         Announcements.getInstance().announceToAll(msg);
   ...
   public void startTimerTask(long time, final boolean checkZero) {
      ...
      this._startTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
         @Override
         public void run() {
            AngelCat.this.eventStart(checkZero ? -1L : (long)(AngelCat.this._template.getPeriod() * 3600000));
         }
