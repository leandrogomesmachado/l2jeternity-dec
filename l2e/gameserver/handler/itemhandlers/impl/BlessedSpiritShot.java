   ...
   public boolean useItem(Playable playable, ItemInstance item, boolean forceUse) {
      if (!playable.isPlayer()) {
         ...
         if (skills == null) {
            ...
            return false;
         } else if (weaponInst == null || weaponItem == null || weaponItem.getSpiritShotCount() == 0) {
            if (!activeChar.getAutoSoulShot().contains(itemId)) {
               activeChar.sendPacket(SystemMessageId.CANNOT_USE_SPIRITSHOTS);
            }

            return false;
         } else if (activeChar.isChargedShot(ShotType.BLESSED_SPIRITSHOTS)) {
            return false;
         } else {
            ...
            if (!gradeCheck) {
               return false;
            } else if (!Config.INFINITE_BLESSED_SPIRIT_SHOT
               ...
               Broadcast.toSelfAndKnownPlayersInRadius(activeChar, new MagicSkillUse(activeChar, activeChar, skills[0].getId(), skills[0].getLvl(), 0, 0), 100);
               return true;
            }
         }
      }
