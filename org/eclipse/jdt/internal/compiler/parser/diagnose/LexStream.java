...
public class LexStream implements TerminalTokens {
   ...
   public LexStream(
      int size, Scanner scanner, int[] intervalStartToSkip, int[] intervalEndToSkip, int[] intervalFlagsToSkip, int firstToken, int init, int eof
   ) {
      this.tokenCache = new LexStream.Token[size];
      this.tokenCacheIndex = 0;
      this.tokenCacheEOFIndex = Integer.MAX_VALUE;
      this.tokenCache[0] = new LexStream.Token();
         ...
         try {
            ...
            if (tokenKind != 60) {
               ...
               if (this.intervalStartToSkip.length != 0 && nextInterval < this.intervalStartToSkip.length && start >= this.intervalStartToSkip[nextInterval]) {
                  ...
                  LexStream.Token token = new LexStream.Token();
   ...
   public LexStream.Token token(int index) {
      if (index < 0) {
         LexStream.Token eofToken = new LexStream.Token();
         eofToken.kind = 60;
   ...
   public String toString() {
      ...
      if (this.currentIndex < 0) {
         ...
         LexStream.Token token = this.token(this.currentIndex);
