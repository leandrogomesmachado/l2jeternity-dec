...
public abstract class AbstractDataResult<T> implements ResultStreamer, Iterator<T> {
   ...
   public T next() {
      if (this.all != null) {
         ...
         com.mysql.cj.result.Row r = this.rows.next();
         if (r == null) {
   ...
   public Iterator<Warning> getWarnings() {
      return this.getStatementExecuteOk().getWarnings().stream().map(w -> new WarningImpl(w)).collect(Collectors.toList()).iterator();
   }
}
