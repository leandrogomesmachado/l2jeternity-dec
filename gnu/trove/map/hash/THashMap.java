...
public class THashMap<K, V> extends TObjectHash<K> implements TMap<K, V>, Externalizable {
   ...
   public boolean equals(Object other) {
      if (!(other instanceof Map)) {
         ...
         return that.size() != this.size() ? false : this.forEachEntry(new THashMap.EqProcedure<>(that));
      }
   }

   @Override
   public int hashCode() {
      THashMap<K, V>.HashProcedure p = new THashMap.HashProcedure();
      this.forEachEntry(p);
   ...
   public Collection<V> values() {
      return new THashMap.ValueView();
   }

   @Override
   public Set<K> keySet() {
      return new THashMap.KeyView();
   }

   @Override
   public Set<java.util.Map.Entry<K, V>> entrySet() {
      return new THashMap.EntryView();
   }
   ...
   public boolean containsValue(Object val) {
      ...
      if (null == val) {
         ...
         while(i-- > 0) {
            if (set[i] != FREE && set[i] != REMOVED && (val == vals[i] || this.equals(val, vals[i]))) {
               return true;
            }
         }
      }
   ...
   final class Entry implements java.util.Map.Entry<K, V> {
      ...
      public boolean equals(Object o) {
         if (!(o instanceof java.util.Map.Entry)) {
            ...
            return THashMap.this.equals(this.getKey(), e2.getKey()) && THashMap.this.equals(this.getValue(), this.getValue());
         }
      }
   ...
   protected class EntryView extends THashMap<K, V>.MapBackedView<java.util.Map.Entry<K, V>> {
      @Override
      public Iterator<java.util.Map.Entry<K, V>> iterator() {
         return new THashMap.EntryView.EntryIterator(THashMap.this);
      }
      ...
      public boolean containsElement(java.util.Map.Entry<K, V> entry) {
         V val = THashMap.this.get(this.keyForEntry(entry));
      ...
      private final class EntryIterator extends TObjectHashIterator {
         ...
         public THashMap<K, V>.Entry objectAtIndex(int index) {
            return THashMap.this.new Entry(THashMap.this._set[index], THashMap.this._values[index], index);
         }
      }
   }
