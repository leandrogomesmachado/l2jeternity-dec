...
public class FieldInfo extends ClassFileStruct implements IBinaryField, Comparable {
   ...
   public static FieldInfo createField(byte[] classFileBytes, int[] offsets, int offset) {
      ...
      if (typeAnnotations != null) {
         ...
         return (FieldInfo)(annotations != null ? new FieldInfoWithAnnotation(fieldInfo, annotations) : fieldInfo);
      }
   }
      ...
      if (numberOfAnnotations <= 0) {
         ...
         for(int i = 0; i < numberOfAnnotations; ++i) {
            TypeAnnotationInfo newInfo = new TypeAnnotationInfo(
               this.reference, this.constantPoolOffsets, readOffset + this.structOffset, runtimeVisible, false
            );
            readOffset += newInfo.readOffset;
            typeAnnos[i] = newInfo;
         }
      ...
      buffer.append('{')
         .append(
            ((modifiers & 1048576) != 0 ? "deprecated " : Util.EMPTY_STRING)
               ...
               + ((modifiers & 64) == 64 ? "volatile " : Util.EMPTY_STRING)
               + ((modifiers & 128) == 128 ? "transient " : Util.EMPTY_STRING)
         )
