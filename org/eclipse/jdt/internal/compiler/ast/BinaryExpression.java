   ...
   public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
      ...
      try {
         if (this.resolvedType.id != 11) {
            ...
            FlowInfo var8 = this.left.analyseCode(currentScope, flowContext, flowInfo).unconditionalInits();
            if ((this.bits & 4032) >> 6 != 2) {
               flowContext.expireNullCheckedFieldInfo();
            }

            this.right.checkNPE(currentScope, flowContext, var8);
            FlowInfo var9 = this.right.analyseCode(currentScope, flowContext, var8).unconditionalInits();
            if ((this.bits & 4032) >> 6 != 2) {
               flowContext.expireNullCheckedFieldInfo();
            }
      ...
      if (leftType != null && rightType != null) {
         ...
         if ((leftIsCast = this.left instanceof CastExpression) || rightIsCast) {
            CastExpression.checkNeedForArgumentCasts(
               scope, operator, operatorSignature, this.left, leftTypeID, leftIsCast, this.right, rightTypeID, rightIsCast
            );
         }
