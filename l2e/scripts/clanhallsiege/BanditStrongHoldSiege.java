...
public final class BanditStrongHoldSiege extends ClanHallSiegeEngine {
   ...
   protected static Map<Integer, BanditStrongHoldSiege.ClanData> _data = new HashMap<>();
   ...
   public String onFirstTalk(Npc npc, Player player) {
      ...
      if (npc.getId() == MESSENGER) {
         if (!this.checkIsAttacker(player.getClan())) {
            ...
            html.setFile(player, "data/scripts/clanhallsiege/" + qn + "/" + player.getLang() + "/35437-00.htm");
   ...
   public String onAdvEvent(String event, Npc npc, Player player) {
      ...
      if (event.startsWith("Register")) {
            ...
            if (this.getAttackers().size() >= 5) {
               ...
               if (arg.length >= 2) {
                  if (arg[1].equals("wQuest")) {
                     if (player.destroyItemByItemId("Bandit Strong Hall Siege", QUEST_REWARD, 1L, player, true)) {
         ...
         BanditStrongHoldSiege.ClanData cd = null;
         if (clan == null) {
            htmltext = "35437-10.htm";
         ...
         if (clan == null) {
            ...
            BanditStrongHoldSiege.ClanData data = _data.get(clan.getId());
         ...
         htmltext = HtmCache.getInstance().getHtm(player, "data/scripts/clanhallsiege/" + qn + "/" + player.getLang() + "/35437-13.htm");
         int i = 0;

         for(Entry<Integer, BanditStrongHoldSiege.ClanData> clanData : _data.entrySet()) {
         ...
         if (clan == null || clan.getLevel() < 4) {
            ...
            } else if (player.getInventory().getInventoryItemCount(TARLK_AMULET, -1) >= 30L) {
         ...
         synchronized(this) {
            ...
            if (!this._firstPhase) {
               ...
               if (_data.size() == 2 && this._hall.getOwnerId() > 0) {
                  ...
                  for(BanditStrongHoldSiege.ClanData data : _data.values()) {
                  ...
                  ThreadPoolManager.getInstance().schedule(new Runnable() {
                     @Override
                     public void run() {
                        for(int doorId : BanditStrongHoldSiege.INNER_DOORS_TO_OPEN) {
                           BanditStrongHoldSiege.this._hall.openCloseDoor(doorId, false);
                        }

                        for(Entry<Integer, BanditStrongHoldSiege.ClanData> e : BanditStrongHoldSiege._data.entrySet()) {
                           BanditStrongHoldSiege.this.doSpawns(e.getKey(), e.getValue());
                        }

                        BanditStrongHoldSiege.this._hall.getSiegeZone().setIsActive(true);
                     }
                  }, 300000L);
               }
            } else {
   ...
   public void startSiege() {
      if (this.getAttackers().size() < 2) {
         ...
         ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
               for(int door : BanditStrongHoldSiege.OUTTER_DOORS_TO_OPEN) {
                  BanditStrongHoldSiege.this._hall.openCloseDoor(door, false);
               }

               BanditStrongHoldSiege.this._hall.getZone().banishNonSiegeParticipants();
   ...
   public void onSiegeStarts() {
      for(Entry<Integer, BanditStrongHoldSiege.ClanData> clan : _data.entrySet()) {
         try {
            BanditStrongHoldSiege.ClanData data = clan.getValue();
   ...
   public final Location getInnerSpawnLoc(Player player) {
      ...
      if (player.getId() == this._hall.getOwnerId()) {
         ...
         BanditStrongHoldSiege.ClanData cd = _data.get(player.getId());
         if (cd != null) {
   ...
   void doSpawns(int clanId, BanditStrongHoldSiege.ClanData data) {
      try {
   ...
   private void fillPlayerList(BanditStrongHoldSiege.ClanData data) {
   ...
   private void registerClan(Clan clan) {
      ...
      BanditStrongHoldSiege.ClanData data = new BanditStrongHoldSiege.ClanData();
   ...
   private final void doUnSpawns(BanditStrongHoldSiege.ClanData data) {
   ...
   private final void removeParticipant(int clanId, boolean teleport) {
      BanditStrongHoldSiege.ClanData dat = _data.remove(clanId);
      if (dat != null) {
   ...
   private void sendRegistrationPageDate(Player player) {
      ...
      msg.setFile(player, "data/scripts/clanhallsiege/" + qn + "/" + player.getLang() + "/35437-14.htm");
   ...
   public static void main(String[] args) {
      new BanditStrongHoldSiege(-1, qn, "clanhallsiege", 35);
   }
