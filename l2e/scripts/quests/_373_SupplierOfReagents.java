...
public class _373_SupplierOfReagents extends Quest {
   ...
   private static final int[][] TEMPERATURE = new int[][]{{1, 100, 1}, {2, 45, 2}, {3, 15, 3}};

   public _373_SupplierOfReagents(int id, String name, String descr) {
      super(id, name, descr);
      this.addStartNpc(30166);
      this.addTalkId(new int[]{30166, 31149});

      for(int[] npc : DROPLIST_COND) {
         this.addKillId(npc[0]);
      }

      this.questItemIds = new int[]{5904, 6317};
   }
   ...
   public String render_urn(QuestState st, String[] page) {
      ...
      if (page == null) {
         html = "<html><body>"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.URN")
            + ":<br><table border=0 width=300><tr><tr><td width=50%><a action=\"bypass -h Quest _373_SupplierOfReagents U_M_MACT\">"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.MIXING_STONE")
            + "</a></td><td></td></tr><tr><td><a action=\"bypass -h Quest _373_SupplierOfReagents U_I_IACT\">"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.INGREDIENTS")
            + "</a></td><td>"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.CURRENT_INGR")
            + "</td></tr><tr><td><a action=\"bypass -h Quest _373_SupplierOfReagents U_C_CACT\">"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.CATALYST")
            + "</a></td><td>"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.CURRENT_CATA")
            + "</td></tr><tr><td><a action=\"bypass -h Quest _373_SupplierOfReagents 31149-5.htm\">"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.SELECT_TEMPERATURE")
            + "</a></td><td>"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.CURRENT_TEMP")
            + "</td></tr><tr><td><a action=\"bypass -h Quest _373_SupplierOfReagents 31149-6.htm\">"
            + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.MIX_INGREDIENTS")
         ...
         for(Object[] ITEM : ITEMS) {
            ...
            if ((item >= 6011 && item <= 6031 || item >= 6320 && item <= 6321) && st.getQuestItemsCount(item) > 0L) {
               ++amt;
               html = html
                  + "<tr><td height=45><img src=icon."
                  + ITEM[1]
                  + " height=32 width=32></td><td width=180>"
                  + ITEM[2]
                  + "</td><td><button value=X1 action=\"bypass -h Quest _373_SupplierOfReagents x_1_"
                  + page[1]
                  + "_"
                  + item
                  + "\" width=40 height=15 fore=sek.cbui92><button value=X10 action=\"bypass -h Quest _373_SupplierOfReagents x_2_"
                  + page[1]
                  + "_"
                  + item
         ...
         if (amt == 0) {
            html = html + "<tr><td>" + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.DO_NOT_HAVE_MATERIALS") + "</td></tr>";
            html = html
               + "</table><center><a action=\"bypass -h Quest _373_SupplierOfReagents urn\">"
               + ServerStorage.getInstance().getString(st.getPlayer().getLang(), "373quest.BACK")
               + "</a></center></body></html>";
         }
      }
   ...
   public String onAdvEvent(String event, Npc npc, Player player) {
      ...
      if (st == null) {
         ...
         if (event.equalsIgnoreCase("30166-4.htm")) {
            ...
            htmltext = this.render_urn(st, null);
            ...
            if (s_event[1].equals("M")) {
               ...
               htmltext = this.render_urn(st, s_event);
            } else if (s_event[2].equals("Retrieve")) {
               int item = 0;
               int qty = 0;
            ...
            if (st.getQuestItemsCount(item) >= (long)var21) {
               String dest;
               String count;
               if (dst.equals("I")) {
                  dest = "ingredient";
               ...
               st.takeItems(item, (long)var21);
               st.set(dest, String.valueOf(item));
               st.set(count, String.valueOf((int)var21));
            ...
            if (st.getInt("mixing") > 0) {
               ...
               if (temp != 0) {
                  ...
                  for(int[] FORMULA : FORMULAS) {
                     if (ingredient == FORMULA[1] && catalyst == FORMULA[3] && iq == FORMULA[2] && cq == FORMULA[4]
                        || ingredient == FORMULA[3] && catalyst == FORMULA[1] && iq == FORMULA[4] && cq == FORMULA[2]) {
                        item = FORMULA[0];
                        break;
                     }
                  }
      ...
      if (partyMember == null) {
         ...
         if (st != null && st.getCond() >= 1) {
            for(int[] i : DROPLIST_COND) {
               if (npc.getId() == i[0]) {
                  st.calcDropItems(this.getId(), i[1], npc.getId(), Integer.MAX_VALUE);
               }
            }
         }
