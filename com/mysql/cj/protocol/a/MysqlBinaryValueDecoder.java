...
public class MysqlBinaryValueDecoder implements ValueDecoder {
   ...
   public <T> T decodeTimestamp(byte[] bytes, int offset, int length, ValueFactory<T> vf) {
      if (length == 0) {
         ...
         int var13 = bytes[offset + 2];
         int var14 = bytes[offset + 3];
         if (length > 4) {
         ...
         return vf.createFromTimestamp(year, var13, var14, hours, minutes, seconds, nanos);
      }
   }
   ...
   public <T> T decodeTime(byte[] bytes, int offset, int length, ValueFactory<T> vf) {
      if (length == 0) {
         ...
         int var12 = bytes[offset + 5];
         int var13 = bytes[offset + 6];
         int var14 = bytes[offset + 7];
         if (negative) {
            days *= -1;
         }
         ...
         return vf.createFromTime(days * 24 + var12, var13, var14, nanos);
      }
   }
