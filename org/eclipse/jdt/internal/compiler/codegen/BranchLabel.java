      ...
      if (this.delegate != null) {
         ...
         if (count >= 1) {
            ...
            if (previousValue < pos) {
               ...
               for(int max = this.forwardReferenceCount; i < max; ++i) {
                  if (refs[i] == pos) {
                     return;
                  }
               }

               if (count >= (i = refs.length)) {
                  System.arraycopy(refs, 0, this.forwardReferences = new int[2 * i], 0, i);
               }
      ...
      if (this.position == -1) {
         ...
         if (this.forwardReferenceCount != 0) {
            isOptimizedBranch = this.forwardReferences[this.forwardReferenceCount - 1] + 2 == this.position
               && this.codeStream.bCodeStream[this.codeStream.classFileOffset - 3] == -89;
