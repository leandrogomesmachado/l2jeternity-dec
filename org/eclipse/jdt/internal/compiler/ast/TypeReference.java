...
public abstract class TypeReference extends Expression {
   ...
   public void getAllAnnotationContexts(int targetType, int info, List allAnnotationContexts) {
      TypeReference.AnnotationCollector collector = new TypeReference.AnnotationCollector(this, targetType, info, allAnnotationContexts);
      this.traverse(collector, null);
   }
   ...
   public void getAllAnnotationContexts(int targetType, int info, List allAnnotationContexts, Annotation[][] annotationsOnDimensions, int dimensions) {
      TypeReference.AnnotationCollector collector = new TypeReference.AnnotationCollector(
         this, targetType, info, allAnnotationContexts, annotationsOnDimensions, dimensions
      );
      this.traverse(collector, null);
      if (annotationsOnDimensions != null) {
         int i = 0;
         ...
         for(int max = annotationsOnDimensions.length; i < max; ++i) {
            ...
            if (annotationsOnDimension != null) {
               ...
               for(int max2 = annotationsOnDimension.length; j < max2; ++j) {
                  annotationsOnDimension[j].traverse(collector, null);
               }
            }
         }
      }
   ...
   public void getAllAnnotationContexts(int targetType, int info, int typeIndex, List allAnnotationContexts) {
      TypeReference.AnnotationCollector collector = new TypeReference.AnnotationCollector(this, targetType, info, typeIndex, allAnnotationContexts);
      this.traverse(collector, null);
   }
   ...
   protected void resolveAnnotations(Scope scope, int location) {
      ...
      if (scope.compilerOptions().isAnnotationBasedNullAnalysisEnabled
         ...
         && (this.resolvedType.tagBits & 108086391056891904L) == 0L
   ...
   protected void checkNullConstraints(Scope scope, Substitution substitution, TypeBinding[] variables, int rank) {
      if (variables != null && variables.length > rank) {
         ...
         if (variable.hasNullTypeAnnotations()
            && NullAnnotationMatching.analyse(variable, this.resolvedType, null, substitution, -1, null, NullAnnotationMatching.CheckMode.BOUND_CHECK)
               .isAnyMismatch()) {
   ...
   static class AnnotationCollector extends ASTVisitor {
      ...
      public AnnotationCollector(
         TypeReference typeReference, int targetType, int info, List annotationContexts, Annotation[][] annotationsOnDimensions, int dimensions
      ) {
         this.annotationContexts = annotationContexts;
         this.typeReference = typeReference;
         this.info = info;
