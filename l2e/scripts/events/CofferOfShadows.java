...
public class CofferOfShadows extends AbstractWorldEvent {
   ...
   public CofferOfShadows(String name, String descr) {
      super(name, descr);
      this.addStartNpc(32091);
      this.addFirstTalkId(32091);
      this.addTalkId(32091);
   ...
   public boolean eventStart(long totalTime) {
      if (!this._isActive && totalTime != 0L) {
         ...
         ServerMessage msg = new ServerMessage("EventCofferOfShadows.START", true);
         ...
         if (totalTime > 0L) {
            this._stopTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
               @Override
               public void run() {
                  CofferOfShadows.this.eventStop();
               }
            }, totalTime);
   ...
   public boolean eventStop() {
      if (!this._isActive) {
         ...
         this._npcList.clear();
         ServerMessage msg = new ServerMessage("EventCofferOfShadows.STOP", true);
         Announcements.getInstance().announceToAll(msg);
   ...
   public String onAdvEvent(String event, Npc npc, Player player) {
      player.getQuestState(this.getName());
   ...
   public void startTimerTask(long time, final boolean checkZero) {
      ...
      this._startTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
         @Override
         public void run() {
            CofferOfShadows.this.eventStart(checkZero ? -1L : (long)(CofferOfShadows.this._template.getPeriod() * 3600000));
         }
