...
public class FortGuardAI extends CharacterAI implements Runnable {
   ...
   private boolean checkAggression(Creature target) {
      if (target == null
         ...
         || target.isAlikeDead()
         || target instanceof FortBallistaInstance
         || target instanceof FortCommanderInstance
         || target instanceof Playable) {
         Player player = null;
         if (target.isPlayer()) {
         ...
         if (player == null || player.getClan() != null && player.getClan().getFortId() == ((Npc)this._actor).getFort().getId()) {
            return false;
         }
      }
   ...
   private void thinkActive() {
      ...
      if (this._actor instanceof DefenderInstance) {
         ((DefenderInstance)this._actor).returnHome();
      } else {
         ((FortCommanderInstance)this._actor).returnHome();
      }
   }
            ...
            for(Creature cha : World.getInstance().getAroundCharacters(this._actor, 1000, 200)) {
               if (cha != null) {
                  if (cha instanceof Npc) {
                     ...
                     && ((Npc)this._actor).getFort().getSiege().checkIsDefender(((Player)cha).getClan())
                     && !this._actor.isAttackingDisabled()
   ...
   private void attackPrepare() {
      ...
      if (this._actor instanceof FortCommanderInstance) {
         sGuard = (FortCommanderInstance)this._actor;
      } else {
         sGuard = (DefenderInstance)this._actor;
      }
      ...
      try {
         ...
         dist_2 = (double)((int)dist) - this._actor.getColRadius();
         range = this._attackRange;
      ...
      if (attackTarget instanceof Player && sGuard.getFort().getSiege().checkIsDefender(((Player)attackTarget).getClan())) {
         ...
         this._actor.setTarget(null);
         this.setIntention(CtrlIntention.IDLE, null, null);
      } else if (!this._actor.isMuted() && dist_2 > (double)range) {
         if (!this.checkSkills(dist_2)) {
            if (!this._actor.isAttackingNow()
               ...
               if (dx * dx + dy * dy > 10000.0
                  ...
                  if (((NpcTemplate)this._actor.getTemplate()).getAI() == "Mystic" || ((NpcTemplate)this._actor.getTemplate()).getAI() == "Priest") {
                     range -= 50;
                  }

                  if (attackTarget.isMoving()) {
                     this.moveTo(attackTarget.getLocation(), (range - 70) / 2);
                  } else {
                     this.moveTo(attackTarget.getLocation(), range / 2);
                  }
               }
            }
         }
         ...
         if (dist_2 <= (double)range) {
            ...
            this._attackTimeout = 300 + GameTimeController.getInstance().getGameTicks();
            if (!this._actor.isMuted() && Rnd.nextInt(100) <= 5 && this.checkSkills(dist_2)) {
               return;
            }
