...
public class SMTPTransport extends Transport {
   ...
   protected SMTPTransport(Session session, URLName urlname, String name, boolean isSSL) {
      ...
      this.useCanonicalHostName = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".sasl.usecanonicalhostname", false);
      if (this.useCanonicalHostName) {
         this.logger.config("use canonical host name");
      }

      SMTPTransport.Authenticator[] a = new SMTPTransport.Authenticator[]{
         new SMTPTransport.LoginAuthenticator(),
         new SMTPTransport.PlainAuthenticator(),
         new SMTPTransport.DigestMD5Authenticator(),
         new SMTPTransport.NtlmAuthenticator()
      };
   ...
   public boolean sasllogin(String[] allowed, String realm, String authzid, String u, String p) throws MessagingException {
      ...
      if (this.saslAuthenticator == null) {
         try {
            ...
            this.logger.log(Level.FINE, "Can't load SASL authenticator", (Throwable)var14);
            return false;
         }
      }
      ...
      try {
         ...
         var20 = this.saslAuthenticator.authenticate(mechs, realm, authzid, u, p);
      } finally {
         this.resumeTracing();
      }
            ...
            try {
               ...
               if (this.sendPartiallyFailed) {
                  ...
                  throw new SMTPSendFailedException(
                     ".", this.lastReturnCode, this.lastServerResponse, this.exception, this.validSentAddr, this.validUnsentAddr, this.invalidAddr
                  );
               }
   ...
   protected void rcptTo() throws MessagingException {
      ...
      for(int i = 0; i < this.addresses.length; ++i) {
         ...
         switch(retCode) {
            ...
            case 251:
               ...
               if (this.reportSuccess) {
                  MessagingException var28 = new SMTPAddressSucceededException(ia, cmd, retCode, this.lastServerResponse);
                  if (mex == null) {
                     mex = var28;
                  } else {
                     mex.setNextException(var28);
                  }
               }
               continue;
            case 450:
            ...
            case 553:
               ...
               MessagingException var26 = new SMTPAddressFailedException(ia, cmd, retCode, this.lastServerResponse);
               if (mex == null) {
                  mex = var26;
               } else {
         ...
         MessagingException var29 = new SMTPAddressFailedException(ia, cmd, retCode, this.lastServerResponse);
         if (mex == null) {
            mex = var29;
         } else {
            mex.setNextException(var29);
         }
      }
   ...
   private void openServer(String host, int port) throws MessagingException {
      ...
      try {
         ...
         this.initStreams();
         int r = -1;
         if ((r = this.readServerResponse()) != true) {
            this.serverSocket.close();
            this.serverSocket = null;
            this.serverOutput = null;
            this.serverInput = null;
   ...
   private abstract class Authenticator {
      ...
      boolean authenticate(String host, String authzid, String user, String passwd) throws MessagingException {
         ...
         try {
            ...
            SMTPTransport.this.logger.log(Level.FINE, "AUTH " + this.mech + " failed", (Throwable)var12);
         } catch (Throwable var13) {
            SMTPTransport.this.logger.log(Level.FINE, "AUTH " + this.mech + " failed", var13);
            thrown = var13;
         } finally {
            if (SMTPTransport.this.noauthdebug && SMTPTransport.this.isTracing()) {
