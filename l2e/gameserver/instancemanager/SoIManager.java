...
public class SoIManager {
   ...
   private static final long SOI_OPEN_TIME = 86400000L;
   private static final Location[] openSeedTeleportLocs = new Location[]{
   ...
   public static long getOpenedTime() {
      return getCurrentStage() != 3 ? 0L : ServerVariables.getLong("SoI_opened", 0L) * 1000L - System.currentTimeMillis();
   }
   ...
   public static void openSeed(long time) {
      if (time > 0L) {
         ...
         ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
               SoIManager.closeSeed();
               SoIManager.setCurrentStage(4);
            }
         }, time);
      }
   }
   ...
   public static void closeSeed() {
      ...
      for(Player ch : ZoneManager.getInstance().getZoneById(200033).getPlayersInside()) {
         if (ch != null) {
            ch.teleToLocation(-183285, 205996, -12896, true);
         }
      }
   }
