...
public class TLongLinkedList implements TLongList, Externalizable {
   ...
   TLongLinkedList.TLongLink tail = this.head;
   ...
   public boolean add(long val) {
      TLongLinkedList.TLongLink l = new TLongLinkedList.TLongLink(val);
      if (no(this.head)) {
         this.head = l;
         this.tail = l;
      } else {
   ...
   void insert(int offset, TLongLinkedList tmp) {
      TLongLinkedList.TLongLink l = this.getLinkAt(offset);
      this.size += tmp.size;
      if (l == this.head) {
         ...
         if (no(l)) {
            ...
            TLongLinkedList.TLongLink prev = l.getPrevious();
   ...
   public TLongLinkedList.TLongLink getLinkAt(int offset) {
      if (offset >= this.size()) {
         return null;
      } else {
   ...
   private static TLongLinkedList.TLongLink getLink(TLongLinkedList.TLongLink l, int idx, int offset) {
      return getLink(l, idx, offset, true);
   }

   private static TLongLinkedList.TLongLink getLink(TLongLinkedList.TLongLink l, int idx, int offset, boolean next) {
   ...
   public boolean remove(long value) {
      ...
      for(TLongLinkedList.TLongLink l = this.head; got(l); l = l.getNext()) {
         if (l.getValue() == value) {
            changed = true;
            this.removeLink(l);
         }
   ...
   private void removeLink(TLongLinkedList.TLongLink l) {
      if (!no(l)) {
         --this.size;
         TLongLinkedList.TLongLink prev = l.getPrevious();
         TLongLinkedList.TLongLink next = l.getNext();
         if (got(prev)) {
            prev.setNext(next);
         } else {
            this.head = next;
   ...
   @Override
   public void reverse() {
      TLongLinkedList.TLongLink h = this.head;
      TLongLinkedList.TLongLink t = this.tail;
      TLongLinkedList.TLongLink l = this.head;

      while(got(l)) {
         TLongLinkedList.TLongLink next = l.getNext();
         TLongLinkedList.TLongLink prev = l.getPrevious();
         TLongLinkedList.TLongLink tmp = l;
         l = l.getNext();
         tmp.setNext(prev);
   ...
   public void reverse(int from, int to) {
      if (from > to) {
         ...
         TLongLinkedList.TLongLink start = this.getLinkAt(from);
         TLongLinkedList.TLongLink stop = this.getLinkAt(to);
         TLongLinkedList.TLongLink tmp = null;
         TLongLinkedList.TLongLink tmpHead = start.getPrevious();
         TLongLinkedList.TLongLink l = start;
   ...
   public void shuffle(Random rand) {
      for(int i = 0; i < this.size; ++i) {
         TLongLinkedList.TLongLink l = this.getLinkAt(rand.nextInt(this.size()));
         this.removeLink(l);
         this.add(l.getValue());
      }
   }
   ...
   public TLongList subList(int begin, int end) {
      if (end < begin) {
         ...
         TLongLinkedList ret = new TLongLinkedList();
         TLongLinkedList.TLongLink tmp = this.getLinkAt(begin);
   ...
   public long[] toArray(long[] dest, int source_pos, int dest_pos, int len) {
      if (len == 0) {
         ...
         TLongLinkedList.TLongLink tmp = this.getLinkAt(source_pos);
   ...
   public boolean forEachDescending(TLongProcedure procedure) {
      for(TLongLinkedList.TLongLink l = this.tail; got(l); l = l.getPrevious()) {
         if (!procedure.execute(l.getValue())) {
            return false;
         }
      }
   ...
   public void fill(int fromIndex, int toIndex, long val) {
      if (fromIndex < 0) {
         ...
         TLongLinkedList.TLongLink l = this.getLinkAt(fromIndex);
   ...
   public int binarySearch(long value, int fromIndex, int toIndex) {
      if (fromIndex < 0) {
         ...
         int from = fromIndex;
         TLongLinkedList.TLongLink fromLink = this.getLinkAt(fromIndex);
         int to = toIndex;

         while(from < to) {
            int mid = from + to >>> 1;
            TLongLinkedList.TLongLink middle = getLink(fromLink, from, mid);
   ...
   public int indexOf(int offset, long value) {
      ...
      for(TLongLinkedList.TLongLink l = this.getLinkAt(offset); got(l.getNext()); l = l.getNext()) {
         if (l.getValue() == value) {
            return count;
         }
   ...
   static class TLongLink {
      ...
      public void setPrevious(TLongLinkedList.TLongLink previous) {
      ...
      public void setNext(TLongLinkedList.TLongLink next) {
         this.next = next;
      }
   }
}
