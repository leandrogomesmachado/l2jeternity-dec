...
public class Ntlm {
   ...
   private byte[] makeDesKey(byte[] input, int off) {
      ...
      return new byte[]{
         (byte)in[off + 0],
         (byte)(in[off + 0] << 7 & 0xFF | in[off + 1] >> 1),
         (byte)(in[off + 1] << 6 & 0xFF | in[off + 2] >> 2),
         (byte)(in[off + 2] << 5 & 0xFF | in[off + 3] >> 3),
         (byte)(in[off + 3] << 4 & 0xFF | in[off + 4] >> 4),
         (byte)(in[off + 4] << 3 & 0xFF | in[off + 5] >> 5),
         (byte)(in[off + 5] << 2 & 0xFF | in[off + 6] >> 6),
         (byte)(in[off + 6] << 1 & 0xFF)
      };
   }
