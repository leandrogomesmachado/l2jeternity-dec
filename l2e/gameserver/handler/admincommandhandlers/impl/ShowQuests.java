...
public class ShowQuests implements IAdminCommandHandler {
   private static final Logger _log = Logger.getLogger(ShowQuests.class.getName());
   private static final String[] ADMIN_COMMANDS = new String[]{"admin_charquestmenu", "admin_setcharquest"};
   private static final String[] _states = new String[]{"CREATED", "STARTED", "COMPLETED"};
      ...
      try (Connection con = DatabaseFactory.getInstance().getConnection()) {
         ...
         if (val[0].equals("full")) {
            ...
            PreparedStatement req = con.prepareStatement(
               "SELECT DISTINCT name FROM character_quests WHERE charId='" + ID + "' AND var='<state>' ORDER by name"
            );
            req.execute();
            ...
            while(rs.next()) {
               replyMSG.append(
                  "<tr><td><a action=\"bypass -h admin_charquestmenu " + target.getName() + " " + rs.getString(1) + "\">" + rs.getString(1) + "</a></td></tr>"
               );
            }
            ...
            PreparedStatement req = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID + "' and name='" + val[1] + "'");
            req.execute();
            ResultSet rs = req.getResultSet();
            ...
            replyMSG.append(
               "Character: <font color=\"LEVEL\">" + target.getName() + "</font><br>Quests with state: <font color=\"LEVEL\">" + val[1] + "</font><br>"
            );
            replyMSG.append("<table width=250>");
            PreparedStatement req = con.prepareStatement(
               "SELECT DISTINCT name FROM character_quests WHERE charId='" + ID + "' and var='<state>' and value='" + val[1] + "'"
            );
            req.execute();
            ResultSet rs = req.getResultSet();
            ...
            if (!exqdb) {
               ...
               replyMSG.append("<center><table width=250><tr><td>Var</td><td>Value</td><td>New Value</td><td>&nbsp;</td></tr>");
               PreparedStatement req = con.prepareStatement("SELECT var,value FROM character_quests WHERE charId='" + ID + "' and name='" + qname + "'");
               req.execute();
               ResultSet rs = req.getResultSet();
         ...
         _log.warning(ShowQuests.class.getSimpleName() + ": " + var28.getMessage());
      }
   }
