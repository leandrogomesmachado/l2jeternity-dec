...
public final class LocalTypeBinding extends NestedTypeBinding {
   ...
   protected void checkRedundantNullnessDefaultRecurse(ASTNode location, Annotation[] annotations, long nullBits, boolean useNullTypeAnnotations) {
      if (!this.isPrototype()) {
         ...
         if (this.enclosingMethod != null) {
            outerDefault = useNullTypeAnnotations ? (long)this.enclosingMethod.defaultNullness : this.enclosingMethod.tagBits & 432345564227567616L;
         }
   ...
   public char[] shortReadableName() {
      ...
      if (this.isAnonymousType()) {
         shortReadableName = CharOperation.concat(
            TypeConstants.ANONYM_PREFIX, this.anonymousOriginalSuperType().shortReadableName(), TypeConstants.ANONYM_SUFFIX
         );
      } else if (this.isMemberType()) {
   ...
   public char[] sourceName() {
      return this.isAnonymousType()
         ? CharOperation.concat(TypeConstants.ANONYM_PREFIX, this.anonymousOriginalSuperType().sourceName(), TypeConstants.ANONYM_SUFFIX)
         : this.sourceName;
   }
   ...
   public String toString() {
      if (this.hasTypeAnnotations()) {
         ...
         return this.isMemberType()
            ? "Local member type : " + new String(this.sourceName()) + " " + super.toString()
            : "Local type : " + new String(this.sourceName()) + " " + super.toString();
      }
   }
