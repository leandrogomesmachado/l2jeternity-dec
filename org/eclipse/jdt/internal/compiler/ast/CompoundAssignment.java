...
public class CompoundAssignment extends Assignment implements OperatorIds {
   ...
   public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
      ...
      FlowInfo var5 = ((Reference)this.lhs).analyseAssignment(currentScope, flowContext, flowInfo, this, true).unconditionalInits();
      if (this.resolvedType.id == 11) {
         LocalVariableBinding local = this.lhs.localVariableBinding();
         if (local != null) {
            var5.markAsDefinitelyNonNull(local);
   ...
   public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
      ...
      ((Reference)this.lhs)
         .generateCompoundAssignment(currentScope, codeStream, this.expression, this.operator, this.preAssignImplicitConversion, valueRequired);
      if (valueRequired) {
         ...
         if (originalLhsType != null && originalExpressionType != null) {
            ...
            if (this.restrainUsageToNumericTypes() && !lhsType.isNumericType()) {
               ...
               if (result == 0) {
                  ...
                  if (this.checkCastCompatibility()
                     && originalLhsType.id != 11
                     && resultType.id != 11
                     && !this.checkCastTypesCompatibility(scope, originalLhsType, resultType, null)) {
                     ...
                     if (expressionIsCast) {
                        CastExpression.checkNeedForArgumentCasts(
                           scope, this.operator, result, this.lhs, originalLhsType.id, false, this.expression, originalExpressionType.id, true
                        );
                     }
