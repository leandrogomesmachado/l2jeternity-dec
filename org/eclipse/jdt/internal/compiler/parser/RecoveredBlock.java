...
public class RecoveredBlock extends RecoveredStatement implements TerminalTokens {
   ...
   public RecoveredElement add(Statement stmt, int bracketBalanceValue, boolean delegatedByParent) {
      ...
      if (this.blockDeclaration.sourceEnd == 0 || stmt.sourceStart <= this.blockDeclaration.sourceEnd) {
         ...
         this.attach(element);
         return (RecoveredElement)(stmt.sourceEnd == 0 ? element : this);
      } else {
               ...
               for(int j = 0; j < i; ++j) {
                  if (updatedStatements[j] instanceof LocalDeclaration) {
                     ...
                     if (local.initialization != null
                        && updatedStatement.sourceStart >= local.initialization.sourceStart
   ...
   public RecoveredElement add(FieldDeclaration fieldDeclaration, int bracketBalanceValue) {
      ...
      if ((fieldDeclaration.modifiers & -17) == 0
         ...
         return (RecoveredElement)(this.blockDeclaration.sourceEnd != 0 && fieldDeclaration.declarationSourceStart > this.blockDeclaration.sourceEnd
            ? this.parent.add(fieldDeclaration, bracketBalanceValue)
            : this);
      } else {
